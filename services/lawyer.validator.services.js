...
const lawyerStep1Validator = () => [
    ...
    body("fullNameUrdu").optional().trim().isLength({ max: 100 })
        .withMessage("Urdu name cannot exceed 100 characters"),
    body("email").optional().trim().isEmail()
        .withMessage("Please enter a valid email"),
    body("phoneNumber").optional().trim().matches(/^\+?[0-9]{10,15}$/)
...
const lawyerStep2Validator = () => [
    body("degree").optional().trim().isLength({ min: 2, max: 100 })
        .withMessage("Degree must be 2-100 characters"),
    body("university").optional().trim().isLength({ min: 2, max: 200 })
        .withMessage("University must be 2-200 characters"),
    body("passingYear").optional().isInt({ min: 1950, max: new Date().getFullYear() })
        .withMessage(`Passing year must be between 1950 and ${new Date().getFullYear()}`),
    body("licenseNo").optional().trim().isLength({ max: 50 })
        .withMessage("License number cannot exceed 50 characters"),
    body("associationBar").optional().trim().isLength({ max: 100 })
        .withMessage("Association bar cannot exceed 100 characters"),
    body("barCity").optional().trim().isLength({ max: 50 })
...
const lawyerStep3Validator = () => [
    body("areasOfPractice").optional().custom((value) => {
    ...
    body("courtJurisdiction").optional().custom((value) => {
        const arr = typeof value === 'string' ? JSON.parse(value) : value;
        if (!Array.isArray(arr) || arr.length === 0) {
    ...
    body("languagesSpoken").optional().custom((value) => {
        ...
        for (const lang of arr) {
            if (!validLangs.includes(lang)) {
                throw new Error(`Invalid language: ${lang}. Valid options: ${validLangs.join(', ')}`);
            }
        }
        return true;
    }),
    body("servicesOffered").optional().custom((value) => {
        const arr = typeof value === 'string' ? JSON.parse(value) : value;
        if (!Array.isArray(arr)) {
...
const lawyerStep4Validator = () => [
    body("availabilityStatus").optional().isIn(Object.values(AVAILABILITY_STATUS))
        .withMessage(`Availability status must be one of: ${Object.values(AVAILABILITY_STATUS).join(', ')}`),
    body("practiceLocations").optional().custom((value) => {
        const arr = typeof value === 'string' ? JSON.parse(value) : value;
        if (!Array.isArray(arr)) {
    ...
    body("professionalBio").optional().trim().isLength({ max: 500 })
        .withMessage("Professional bio cannot exceed 500 characters")
];

// Step 5: Documents
const lawyerStep5Validator = () => [
    cnicValidator()
];
