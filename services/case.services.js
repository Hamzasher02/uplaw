...
import CaseInvitation, {
  INVITATION_STATUS,
} from "../model/caseinvitation.model.js";
...
async function _handleVoiceNoteUpload(voiceNoteFile) {
  ...
  console.log(
    "[Case Service] Voice note file received:",
...
export async function createCaseService(
  ...
  if (caseData.preferredLanguages !== undefined) {
    caseData.preferredLanguages = parseJsonField(
      caseData.preferredLanguages,
      [],
    );
  }
...
export async function getClientCasesService(clientId, status = null) {
  ...
  const matchingProfiles = await LawyerProfile.find({
    ...
    .select(
      "userId areasOfPractice yearsOfExperience city courtJurisdiction languagesSpoken professionalBio",
    )
    .lean();
  ...
  const casesWithLawyers = cases.map((caseDoc) => {
    ...
    const suggestedLawyersRaw = validProfiles
      .filter(
        (profile) =>
          profile.areasOfPractice &&
          profile.areasOfPractice.includes(caseDoc.category),
      )
...
export async function getSuggestedLawyersService(caseId, clientId) {
  ...
  const lawyers = matchingProfiles
    ...
    .map((profile) =>
      mapProfileToLawyer(profile, ["courtJurisdiction", "languagesSpoken"]),
    );
...
export async function inviteLawyersService(caseId, lawyerIds, clientId) {
  ...
  if (caseDoc.clientId.toString() !== clientId) {
    throw new UNAUTHORIZED(
      "You do not have permission to invite lawyers to this case",
    );
  }
...
export async function getLawyerReceivedCasesService(lawyerId, status = null) {
  ...
  const invitations = await CaseInvitation.find(query)
    .populate({
      path: "caseId",
      select:
        "title description category budgetRange province district court urgency preferredLanguages status proposalCount voiceNote createdAt",
      populate: {
        path: "clientId",
        select: "fullName profilePicture",
      },
