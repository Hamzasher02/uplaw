...
export const generateAndSendOtpService = async (
  ...
  const recentOtp = await OTP.findOne({
    ...
    purpose,
    createdAt: {
      $gt: new Date(Date.now() - OTP_CONFIG.RESEND_COOLDOWN_SECONDS * 1000),
    },
  });
 ...
 * Verify OTP
 */
export const verifyOtpService = async (userId, otp, email, purpose) => {
  ...
  if (!otpRecord) {
    throw new BAD_REQUEST("OTP expired or invalid.");
  }
